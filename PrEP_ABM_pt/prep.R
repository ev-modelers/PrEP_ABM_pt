...
#  }
  ...
  ind4 <- dat$attr$prep.ind.uai.nmain
  ...
  idsNoIndic<-1
  # SET eliginility
  if (dat$param$senario =='WHO'){
    idsNoIndic <- which(ind2 < twind | is.na(ind2)) 
    base.cond.no <- which(active == 0 | diag.status == 1)
    idsNoIndic <- union(idsNoIndic, base.cond.no)
    idsIndic <- which(ind2 >= twind )
    base.cond.yes <- which(active == 1 & diag.status == 0)
    idsIndic <- intersect(idsIndic, base.cond.yes)
  }
  
  if (dat$param$senario =='EACS'){
    idsNoIndic <- which(ind4 < twind | is.na(ind4)) 
    base.cond.no <- which(active == 0 | diag.status == 1)
    idsNoIndic <- union(idsNoIndic, base.cond.no)
    idsIndic <- which(ind4 >= twind )
    base.cond.yes <- which(active == 1 & diag.status == 0)
    idsIndic <- intersect(idsIndic, base.cond.yes)
  }
  
  
  if (dat$param$senario =='PNHS'){
    idsNoIndic <- which(ind1 < twind | is.na(ind1)) 
    base.cond.no <- which(active == 0 | diag.status == 1)
    idsNoIndic <- union(idsNoIndic, base.cond.no)
  ...
  if (dat$param$senario =='US'){
    base.cond.no <- which(active == 0 | diag.status == 1)
    idsNoIndic <- base.cond.no
    base.cond.yes <- which(active == 1 & diag.status == 0)
    idsIndic <- base.cond.yes
  }
